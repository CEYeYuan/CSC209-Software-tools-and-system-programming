...
/*
  When passing an array into a function, by default is passing a pointer
  to the first element of the array, but for the struct, it's copying
  the entire struct (including the possible array member).
...
void change(struct student *s) {
    strcpy((*s).first_name, "Adam");//. has the precedence over *
    //alternate syntax: s->gpa=4.0
    (*s).gpa = 4.0;
}
